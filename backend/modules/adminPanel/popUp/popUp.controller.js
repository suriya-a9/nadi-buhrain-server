...
exports.addPopUp = async (req, res, next) => {
    try {
        const { title, totalPoints, questions, allowedAccountTypes, targetUserId } = req.body;
        const popUpQuestionnaire = await PopUpQuestionnaire.create({
            title,
            totalPoints,
            questions,
            allowedAccountTypes,
            targetUserId: Array.isArray(targetUserId) ? targetUserId : [targetUserId]
        })
...
exports.editPopup = async (req, res, next) => {
    try {
        ...
        if (updateFields.targetUserId) {
            updateFields.targetUserId = Array.isArray(updateFields.targetUserId)
                ? updateFields.targetUserId
                : [updateFields.targetUserId];
        }
...
exports.questionnaireDetail = async (req, res, next) => {
    try {
        ...
        const questionnaireData = await PopUpQuestionnaire
            .findById(id)
            .populate({
                path: "targetUserId",
    ...
    try {
        ...
        const listData = await PopUpQuestionnaire.find({
            ...
            $or: [
                ...
                { allowedAccountTypes: { $size: 0 }, targetUserId: null }
            ]
        }).sort({ createdAt: -1 });
