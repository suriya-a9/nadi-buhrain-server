...
export default function Enquiry() {
    ...
    const filteredEnquiry = enquiry.filter(s =>
        String(s.name || "").toLowerCase().includes(search.toLowerCase()) ||
        String(s.email || "").toLowerCase().includes(search.toLowerCase()) ||
        String(s.phone || "").toLowerCase().includes(search.toLowerCase())
    );
    const totalPages = Math.ceil(filteredEnquiry.length / itemsPerPage);
    const paginatedEnquiry = filteredEnquiry.slice(
    ...
    return (
        <div>
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2 mb-4">
                <h2 className="text-[25px] font-bold mb-6 text-textGreen">Enquiry List</h2>
